...
This file adapts the FastAPI backend to work as a Vercel serverless function.
"""
...
# In-memory databases (singleton for serverless)
class UserDatabase:
    _instance = None
    users = {}
...
class SetorDatabase:
    ...
    def _initialize(self):
        ...
        self.add_setor('4', {'id': '4', 'name': 'Setor Desenvolvimento', 'code': 'DEV', 'departamento_id': '2'})
        self.add_setor('5', {'id': '5', 'name': 'Setor Vendas Internas', 'code': 'VINT', 'departamento_id': '3'})
        self.add_setor('6', {'id': '6', 'name': 'Setor Vendas Externas', 'code': 'VEXT', 'departamento_id': '3'})
...
class DepartamentoDatabase:
    ...
    def _initialize(self):
        self.departamentos = {}
        self.add_departamento('1', {'id': '1', 'name': 'Departamento de RH', 'code': 'RH', 'filial_id': '1'})
        self.add_departamento('2', {'id': '2', 'name': 'Departamento de TI', 'code': 'TI', 'filial_id': '1'})
        self.add_departamento('3', {'id': '3', 'name': 'Departamento de Vendas', 'code': 'VEN', 'filial_id': '2'})
        self.add_departamento('4', {'id': '4', 'name': 'Departamento de Marketing', 'code': 'MKT', 'filial_id': '2'})
...
class FilialDatabase:
    ...
    def _initialize(self):
        ...
        self.add_filial('3', {'id': '3', 'name': 'Filial Belo Horizonte', 'code': 'BH'})
...
async def get_weather(latitude: float, longitude: float):
    try:
        async with httpx.AsyncClient() as client:
            ...
            response.raise_for_status()
            data = response.json()
            return {"temperature": data.get("current_weather", {}).get("temperature")}
...
async def get_metrics(
    ...
    try:
        ...
        if period == "annual":
            ...
            temporal_data = pd.merge(temporal_orcado, temporal_realizado, on='ano', how='outer').fillna(0)
            ...
            temporal_data = pd.merge(temporal_orcado, temporal_realizado, on='period', how='outer').fillna(0)
        elif period == "daily":
            ...
            temporal_data = pd.merge(temporal_orcado, temporal_realizado, on='data', how='outer').fillna(0)
        ...
        dre_data = pd.merge(dre_orcado, dre_realizado, on='codigoConta', how='outer').fillna(0)
...
async def chat_with_ai(chat_message: ChatMessage, current_user: User = Depends(get_current_user)):
    if not model:
        raise HTTPException(status_code=503, detail="Chat functionality is disabled. GEMINI_API_KEY not configured.")
...
async def debug_databases(current_user: User = Depends(get_current_user)):
    ...
    return {
        "users": {"count": len(users_db.users), "emails": list(users_db.users.keys())},
        "files": {"count": len(files_db.files), "names": list(files_db.files.keys())},
        "excel_data": {
        ...
        "setores": {"count": len(setores_db.setores), "ids": list(setores_db.setores.keys())},
        "departamentos": {"count": len(departamentos_db.departamentos), "ids": list(departamentos_db.departamentos.keys())},
        "filiais": {"count": len(filiais_db.filiais), "ids": list(filiais_db.filiais.keys())}
    }
